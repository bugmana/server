...
##### Database server #####
###########################
docker_path="/opt/mangos1/database";
repo_path="${PWD%%docker}";

docker-compose -f "${repo_path}/docker/docker-compose.yml" up -d mysql
...
import=( $( find "../../database/Realm/Updates/Rel21" -maxdepth 1 -iname '*.sql' -printf "%f\n" | sort -n) );
if [ ! -z "${import}" ];
then
    for sql in "${import[@]}";
    do
        echo "Importing Realm update ${sql}";
        root_command "${repo_path}" "SOURCE -f ${docker_path}/Realm/Updates/Rel21/${sql}" "realmd";
    done
fi
...
user_command "${repo_path}" "SOURCE ${docker_path}/Character/Setup/characterLoadDB.sql" "character1"

import=( $( find "../../database/Character/Updates/Rel21" -maxdepth 1 -iname '*.sql' -printf "%f\n" | sort -n) );
if [ ! -z "${import}" ];
then
    for sql in "${import[@]}";
    do
        echo "Importing Character update ${sql}";
        root_command "${repo_path}" "SOURCE ${docker_path}/Character/Updates/Rel21/${sql}" "character2";
    done
fi
# find "${repo_path}/Character/Updates/Rel21" -maxdepth 1 -iname '*.sql' -exec bash -c 'root_command "'${repo_path}'" "SOURCE $1" "character1"' _ {} \;

##########################
##### World Database #####
##########################
root_command "${repo_path}" "SOURCE ${docker_path}/World/Setup/mangosdCreateDB.sql" "" || echo "Realm DB already exists";
user_command "${repo_path}" "SOURCE ${docker_path}/World/Setup/mangosdLoadDB.sql" "mangos1"

import=( $( find "../../database/World/Setup/FullDB" -maxdepth 1 -iname '*.sql' -printf "%f\n") );
if [ ! -z "${import}" ];
then
    for sql in "${import[@]}";
    do
        echo "Importing World Setup ${sql}";
        root_command "${repo_path}" "SOURCE ${docker_path}/World/Setup/FullDB/${sql}" "mangos1";
    done
fi
# find "${repo_path}/World/Setup/FullDB" -maxdepth 1 -iname '*.sql' -exec bash -c 'root_command "'${repo_path}'" "SOURCE $1" "mangos1"' _ {} \;

import=( $( find "../../database/World/Updates/Rel21" -maxdepth 1 -iname '*.sql' -printf "%f\n" | sort -n) );
if [ ! -z "${import}" ];
then
    for sql in "${import[@]}";
    do
        echo "Importing World Update ${sql}";
        root_command "${repo_path}" "SOURCE ${docker_path}/World/Updates/Rel21/${sql}" "mangos1";
    done
fi
# find "${repo_path}/World/Updates/Rel21" -maxdepth 1 -iname '*.sql' -exec bash -c 'root_command "'${repo_path}'" "SOURCE $1" "mangos1"' _ {} \;
